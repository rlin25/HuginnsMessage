...
Every decision in this document was made before the code was written, using a Socratic design methodology: Claude posed structured questions about each design area, challenged weak reasoning, and presented alternatives. Decisions were locked only when the reasoning was explicit and all alternatives were documented with rejection rationale. All architectural judgment belonged to the developer — Claude pressed on the reasoning; the developer made the calls.

Rejected alternatives are documented alongside every accepted decision. This is not formality. It is evidence of how the tradeoff was evaluated. When a future version changes a decision, the rejected alternative documents what was already considered — preventing the project from re-arguing resolved questions. When a reviewer reads this document, the rejected alternatives are the clearest signal of how design tradeoffs were weighed.

This document is the counterpart to the interface contract (`INTERFACE_CONTRACT.md`). The interface contract defines what each component does. This document explains why it does it that way.
