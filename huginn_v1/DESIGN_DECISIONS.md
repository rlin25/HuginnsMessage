...
Every major decision in Huginn was made through a structured Socratic design process before any code was written. Claude was used as a Socratic partner — asking questions, presenting alternatives, challenging weak reasoning, and formalizing the outcomes. Every architectural judgment belonged to the developer.

The decisions below document not just what was decided, but why — and what was explicitly considered and rejected. That rejected-alternative record is the most important part. It demonstrates that each decision was made consciously, that alternatives were considered, and that the reasoning is traceable when a consequence of a decision surfaces later. A design document that records only decisions without rejected alternatives is a record of outcomes, not of thinking.

This document and the interface contract are the artifacts that carry intent across versions. The code is regenerated from scratch for each version. These documents are not.
